...
{
    public class Service:RepositoryBase<Model.Speed_Debian>, Speed_Debian.IService
    {
        ...
        public async Task<List<Model.Speed_Debian>> GetSpeedDebianByDateAsync(DateTime resultReceivedDate)
        {
            // Truy vấn các dữ liệu có trường TsResultReceived trùng với tham số truyền vào
            return await _dbContext.Set<Model.Speed_Debian>()
                                 .Where(s => s.TsResultReceived.Date == resultReceivedDate.Date)
                                 .ToListAsync();
        }
    }
}
