...
{
    public class Net_UC_LinhVucController : ApiControllerBase<Net_UC_LinhVuc>
    {
        private readonly IServiceWrapper _service;
        private readonly ILogger<Net_UC_LinhVucController> _logger;
        private readonly IUserProvider _userProvider;
        public Net_UC_LinhVucController(IServiceWrapper service, IUserProvider userProvider, ILogger<Net_UC_LinhVucController> logger) :base(service, logger)
        {
            _logger = logger;
            _service = service;
        ...
        public async Task<IActionResult> GetListPagedByType(int page = 1, int pageSize = 10, int totalLimitItems = 500, int type = 0)
        {
            try
            {
                ...
                var items = await _service.Net_UC_LinhVucs.GetPagedAsync(page, pageSize, totalLimitItems, search);
