...
{
    public class Net_CableManagementController : ApiControllerBase<Net_CableManagement>
    {
        private readonly IServiceWrapper _service;
        private readonly ILogger<Net_CableManagementController> _logger;
        private readonly IUserProvider _userProvider;
        public Net_CableManagementController(IServiceWrapper service, IUserProvider userProvider, ILogger<Net_CableManagementController> logger) :base(service, logger)
        {
            _logger = logger;
            _service = service;
        ...
        public async Task<IActionResult> GetListAsync(int page = 1, int pageSize = 10, int totalLimitItems = 500)
        {
            try
            {
                ...
                var items = await _service.Net_CableManagement.GetListAsync(page, pageSize, totalLimitItems);
        ...
        public async Task<IActionResult> CheckDuplicateAttributes(Guid? id, string code)
        {
            try
            {
                ...
                var result = await _service.Net_CableManagement.IsDupicateAttributesAsync(id, code);
