...
{
    public class NetUsecase_RunController : ApiControllerBase<NetUsecase_Run>
    {
        private readonly IServiceWrapper _service;
        private readonly ILogger<NetUsecase_RunController> _logger;
        private readonly IUserProvider _userProvider;
        public NetUsecase_RunController(IServiceWrapper service, IUserProvider userProvider, ILogger<NetUsecase_RunController> logger) :base(service, logger)
        {
            _logger = logger;
            _service = service;
        ...
        public async Task<IActionResult> GetListPagedByType(int page = 1, int pageSize = 10, int totalLimitItems = 500, int type = 0)
        {
            try
            {
                ...
                var items = await _service.NetUsecase_Runs.GetListAsync(page, pageSize, totalLimitItems);
