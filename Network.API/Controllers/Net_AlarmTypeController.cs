...
{
    public class Net_AlarmTypeController : ApiControllerBase<Net_AlarmType>
    {
        private readonly IServiceWrapper _service;
        private readonly ILogger<Net_AlarmTypeController> _logger;
        private readonly IUserProvider _userProvider;
        public Net_AlarmTypeController(IServiceWrapper service, IUserProvider userProvider, ILogger<Net_AlarmTypeController> logger) :base(service, logger)
        {
            _logger = logger;
            _service = service;
        ...
        public async Task<IActionResult> GetListAsync(int page = 1, int pageSize = 10, int totalLimitItems = 500)
        {
            try
            {
                ...
                var items = await _service.Net_AlarmType.GetListAsync(page, pageSize, totalLimitItems);
        ...
        public async Task<IActionResult> CheckDuplicateAttributes(Guid? id, string code)
        {
            try
            {
                ...
                var result = await _service.Net_AlarmType.IsDupicateAttributesAsync(id, code);
