...
{
    public class Net_CurenAlarmController : ApiControllerBase<Net_CurenAlarm>
    {
        private readonly IServiceWrapper _service;
        private readonly ILogger<Net_CurenAlarmController> _logger;
        private readonly IUserProvider _userProvider;
        public Net_CurenAlarmController(IServiceWrapper service, IUserProvider userProvider, ILogger<Net_CurenAlarmController> logger) :base(service, logger)
        {
            _logger = logger;
            _service = service;
        ...
        public async Task<IActionResult> GetListAsync(int page = 1, int pageSize = 10, int totalLimitItems = 500)
        {
            try
            {
                ...
                var items = await _service.Net_CurenAlarm.GetListAsync(page, pageSize, totalLimitItems);
